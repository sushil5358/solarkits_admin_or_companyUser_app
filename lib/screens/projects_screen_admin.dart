...
import '../appColors.dart';
import '../constants.dart';
import '../controller/project_order_cu_controller.dart';
import '../models/project_order_model.dart';
...
class _ProjectsManagementScreenState extends State<ProjectsManagementScreen> {
  final ProjectOrderCuController controller = Get.put(
      ProjectOrderCuController());
  final ScrollController _scrollController = ScrollController();


  @override
  void dispose() {
    Get.delete<ProjectOrderCuController>();
  ...
  // ---- Helper: Convert ProjectOrderModel to map for UI ----
  Map<String, dynamic> _toOrderMap(ProjectOrderModel order) {
    int stage = 0;
    if (order.orderCreated) stage = 1;
    if (order.atWarehouse) stage = 2;
    if (order.willDeliver) stage = 3;
    if (order.inTransit) stage = 4;
    if (order.delivered) stage = 5;
    ...
    if (order.orderDate != null) {
      final created = order.orderDate!;
      final expected = order.expectedDate ?? 0;
      final delivery = created.add(Duration(days: expected));
      deliveryDate = DateFormat('yyyy-MM-dd').format(delivery);
    }

    final formatter = NumberFormat('#,##,###');
    final grandTotalStr = '₹${formatter.format(order.grandTotal)}';

    return {
      'id': order.id,
      'orderNo': order.orderId ?? order.quoteNumber ?? '',
      'customer': order.name,
      ...
      'status': statusText,
      'statusColor': '#${statusColor.value.toRadixString(16).substring(2)}',
      'date': order.createdAt != null ? DateFormat('yyyy-MM-dd').format(
          order.createdAt!) : '',
      'deliveryDate': deliveryDate,
      'kw': order.kilowatt.toString(),
      'panelCount': order.numberOfSolarPanel.toString(),
      'panelTech': order.technology?.name ?? '',
      'comboType': order.kitType ?? '',
      'grandTotal': grandTotalStr,
      'orderStage': stage,
      'paymentCompleted': !order.paymentPending,
    };
  }

  // ---- Build card ----
  Widget _buildOrderCard(ProjectOrderModel order, int index) {
    final orderMap = _toOrderMap(order);
    return _buildOrderCardFromMap(orderMap, index);
  }

  Widget _buildOrderCardFromMap(Map<String, dynamic> order, int index) {
    Color statusColor;
    try {
      statusColor =
              ...
              children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Icon(
                                  Icons.phone, size: 12, color: AppColors.grey),
                              const SizedBox(width: 4),
                              Text(
                                order['mobile'],
                                style: TextStyle(
                ...
                Row(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Icon(Icons.local_shipping, size: 10,
                            color: AppColors.grey),
                        const SizedBox(width: 4),
                        Text(
                          'Expected: ${order['deliveryDate']}',
                ...
                children: [
                  if (idx > 0)
                    Expanded(
                      child: Container(
                        height: 2,
                        color: isCompleted ? AppColors.successGreen : AppColors
                            .lightGrey,
                      ),
                    ),
                  Container(
                    ...
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      color: isCompleted || isCurrent
                          ? (isCompleted ? AppColors.successGreen : AppColors
                          .successGreen)
                          : AppColors.lightGrey,
                    ),
                    child: Icon(
                  ...
                  children: [
                    ...
                    Obx(() {
                      return MultiSelectDialogField(
                        items: controller.districtList.map((dis) =>
                            MultiSelectItem(dis, dis.name)).toList(),
                    ...
                    Obx(() {
                      return MultiSelectDialogField(
                        items: controller.subCategoryList.map((sub) =>
                            MultiSelectItem(sub, sub.name)).toList(),
                        title: const Text('Select Sub Category'),
                        onConfirm: (values) {
                    ...
                    Obx(() {
                      return MultiSelectDialogField(
                        items: controller.projectTypeList.map((pt) =>
                            MultiSelectItem(pt, pt.id)).toList(),
                        title: const Text('Select Project Type'),
                        onConfirm: (values) {
                    ...
                    Obx(() {
                      return MultiSelectDialogField(
                        items: controller.subProjectTypeList.map((spt) => MultiSelectItem(spt, spt.name)).toList(),
                        title: const Text('Select Sub Project Type'),
                        onConfirm: (values) {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        _buildTrackingRow('Order Created', order['date'],
                            order['orderStage'] >= 1),
                        _buildTrackingRow('At Warehouse', order['date'],
                            order['orderStage'] >= 2),
                        _buildTrackingRow(
                            'Ready to Deliver', order['deliveryDate'],
                            order['orderStage'] >= 3),
                        _buildTrackingRow('In Transit', order['deliveryDate'],
                            order['orderStage'] >= 4),
                        _buildTrackingRow('Delivered', order['deliveryDate'],
                            order['orderStage'] >= 5),
                      ],
                    ),
                  ),
